---
name: write-task
description: Use when the user says "write-task", "write a task", "write a ticket", "create a backlog ticket", or asks for a ClickUp / User-Story work item for a MediaServing feature — a backlog ticket that must respect the Ramaaz Backlog Ticket Standard.
---

# write-task — Backlog ticket writer (MediaServing)

Produce a backlog ticket that fully complies with the **Backlog Ticket Standard**
...
> **Not the same thing as `/start-ticket`.** This skill writes a *ClickUp backlog
> ticket* (a User Story / Work Item for the product backlog). Engineering Workflow v1
> artifacts (`_specs/<ticket>/ticket.md`, `research.md`, `spec.md`, …) are a separate
> track and are never written by this skill.

## How to use

1. Take the feature the user describes (e.g. "add a signed-URL check to the upload route").
...
6. **Always write the finished ticket to `Tickets/<Verb-Object-Title>.md`** (repo-root
   `Tickets/` folder, Title-Case-with-hyphens filename matching the existing convention
   there), then also output it in the chat as one markdown block the user can paste into
   ClickUp.
...
> space **TryDosProject** `901811062695`) — the same workspace MediaServing tickets are
> filed in (see `.claude/docs/clickup-sync-design.md`). Only ever use option labels listed
> in the **ClickUp field reference** at the bottom of this skill — never invent a status,
...
| Backbone (module) | ✔️ | The ClickUp module folder the ticket belongs to: `Admin Dashboard`, `Seller DashBoard`, `Story`, `Chat`, `Client`, `Auth`, `MobApp`, `QA`. (Not a custom field — it is the folder; pick the closest module. MediaServing work usually lands under the module that *consumes* the media API — e.g. `Story` for story upload/playback, `Auth` for gated upload. If none fits, pick the closest and flag it with ⚠️.) |
...
**only** legal option labels. Custom-field **IDs** (for writes) are listed in
`.claude/docs/clickup-sync-design.md §3.3`.
...
### Pushing the ticket to ClickUp (optional)
Only when the user explicitly asks to create the task. `scripts/clickup_create_task.py`
is the only place ClickUp write logic lives — check for a duplicate first, then create:
```bash
python scripts/clickup_create_task.py --list 901818662901 --find-by-name "<Title>"
python scripts/clickup_create_task.py --list 901818662901 --name "<Title>" \
  --description-file "Tickets/<Verb-Object-Title>.md" --status draft
```
